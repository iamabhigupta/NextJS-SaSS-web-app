...
const FormLayoutsBasic2 = ({ setMainValues, mainValues }) => {
  // ** States

  useEffect(() => {
    setValues(mainValues)
  }, [mainValues])
  const [values, setValues] = useState(mainValues)
  ...
  const handleChange = (name, value) => {
    ...
    setMainValues({ ...values, [name]: value })
  }

  const handleChangeEvent = event => {
    setValues({ ...values, [event.target.name]: event.target.value })
    setMainValues({ ...values, [event.target.name]: event.target.value })
  }
  ...
  return (
    <Card>
      ...
      {storeData.length > 0 && (
        <Link
          target='_blank'
          href={process.env.NEXT_PUBLIC_STORE_ADDRESS + storeData[0].site_name}
          underline='hover'
          sx={{
            position: 'absolute',
            top: '18%',
        ...
        <form onSubmit={e => e.preventDefault()}>
          <Grid container spacing={5}>
            ...
            <Grid item xs={12}>
              ...
              <DropzoneWrapper>
                <Grid item xs={12}>
                  <FileUploaderMultiple fileId={text => handleChange('file', text)} />
                </Grid>
              </DropzoneWrapper>
            </Grid>
...
export default FormLayoutsBasic2
